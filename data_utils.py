...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text):
        ...
        # return text_norm
        return self.text_to_embeddings(text, "hi").detach().clone()
...
class TextAudioCollate:
    """Zero-pads model inputs (BERT embeddings) and audio"""
    ...
    def __call__(self, batch):
        ...
        max_text_len = max([x[0].size(0) for x in batch])   # len
        emb_dim = batch[0][0].size(1)                       # embedding dimension
        ...
        text_padded = torch.FloatTensor(len(batch), max_text_len, emb_dim).zero_()
