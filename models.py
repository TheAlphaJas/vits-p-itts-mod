...
class IndicBERTTextEncoder(nn.Module):
    """
    A modified VITS2 TextEncoder designed to work with pre-computed embeddings
    from a model like IndicBERT, instead of an internal vocabulary embedding.

    This module removes the nn.Embedding layer and expects a tensor of
    contextual embeddings as input.
    """
    def __init__(
        self,
        # The `n_vocab` parameter is removed as it's no longer needed.
        out_channels,
        hidden_channels,
        filter_channels,
        n_heads,
        ...
        # --- MODIFICATION START ---
        # The `self.emb = nn.Embedding(...)` layer has been completely removed.
        # The model now expects embeddings to be passed directly to the forward method.
        # The `hidden_channels` parameter MUST now match the dimension of the
        # incoming embeddings from the IndicBERTProcessor's projection layer.
        # --- MODIFICATION END ---
    ...
    def forward(self, x, x_lengths, g=None):
        """
        The forward pass now accepts the embedding tensor directly.

        Args:
            x (torch.Tensor): The input tensor of contextual embeddings.
                              Shape: [batch_size, sequence_length, hidden_channels]
            x_lengths (torch.Tensor): The lengths of the input sequences.
                                      Shape: [batch_size]
            g (torch.Tensor, optional): Speaker embedding. Defaults to None.
        """
        # --- MODIFICATION START ---
        # The original embedding lookup is removed:
        # x = self.emb(x) * math.sqrt(self.hidden_channels)
        # The input `x` is now the output from your IndicBERTProcessor, which is
        # already a tensor of shape [batch, seq_len, hidden_channels].
        # We just need to transpose it for the convolutional layers.
        x = torch.transpose(x, 1, -1)  # [b, h, t]
        # --- MODIFICATION END ---
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc_p = IndicBERTTextEncoder(
            inter_channels,
            hidden_channels,
            filter_channels,
            n_heads,
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        #print("x size - ", x.size)
        #print("x lengths size - ", x_lengths.size)
